...
//include("controller/filesystem.php");
...
//echo "select unit from pro_folder where id=$cid";exit;
$prounitid=returnQueryValue("select unit from pro_folder where id=$cid","unit");
$unitid=$prounitid;


//echo $prounitid;exit;
$unitname=returnQueryValue("select name from pro_dept_units where id=$prounitid","name");
$deptid=returnQueryValue("select deptid from pro_dept_units where id=$prounitid","deptid");
...
$foldername=returnQueryValue("select name from pro_folder where id=$cid","name");
$parentid=returnQueryValue("select parentfolder from pro_folder where id=$cid","parentfolder");
$foldpack="";
if($parentid<1){
	$foldpack="../".$foldername."/";
}
else{
	$foldername2=returnQueryValue("select name from pro_folder where id=$parentid","name");
	$foldpack="../".$foldername2."/".$foldername."/";
}
...
//echo $viewerid;exit;
$canview=getFolderPermission($cid,$viewerid,"canread");
//echo $canview;exit;
?>
			...
			<div class="main-content" id="divmaincontent">
				...
				<div class="main-content-inner">
					...
					<div class="alert alert-danger" style="display:none;" id="diverror"><i class="ace-icon fa"><img src="img/stop.png" style="width:60px;height:60px;"></i>
											...
											<span style="font-size:18px;">You do not have 'Read' Permission to this resources.</span><br>
											Contact System Admin.<br>
											<br />
										</div>

					<div class="page-content" id="divpass">
...
$deptidx=returnQueryValue("select deptid from pro_dept_units where id=$unitid","deptid");
 ...
 ?>
						...
						<h2>
							...
							<?php echo "//:$compname/$deptname/$unitname/$foldpack"; ?>
							</h2>
				...
				<?php 
				$qry="select * from pro_folder where deleted='N' and parentfolder=$cid";
				$res=mysql_query($qry);
				...
				if($num>0){
					...
					do{
						...
						echo "<div class='explorer_folder'><center><img src='img/folder.png' style='width:100px;height:100px;'><br>$cmp</center><span style='font-size:9px;'><a href='javascript:explore(\"$compid\");'>Open</a> | <a href='rename.php?id=$compid&pid=$cid'>Rename</a> | <a href='permissions.php?fid=$compid'>Permission</a> | <a href='javascript:deletefolder(\"$compid\");'>Delete<a></span></div>";
...
//alert(canview);
	if(canview=="N"){
		//document.getElementById("divcontentx").innerHTML="<div class='alert alert-danger><button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button><strong><i class='ace-icon fa fa-times'></i>Access Denied</strong>File Read Permission error<br /></div>";
...
function createFolderInUnit(){
	...
	$.post("controller/filesystem.php", {unitid: unitid,act:"createfolder", txtfolderinput: txtfolderinput, cboschema: cboschema, parentfolder: parentfolder  },
						   function (data) {
							   ...
							   if (data.length > 0) {
								  ...
								  // alert(data);
								   ...
								   if(data=="1"){
										window.location="explorer_folder.php?id="+parentfolder;
								   }
							   ...
							   if (data.length > 0) {
								   ...
								   if(data=="Y"){
									  // window.location="viewer.php?id="+cid;
									   window.open("viewer.php?id="+cid,'_blank');
								   }
								   else{
